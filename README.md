...
[🚀 在线只读交互演示 (Vercel Showcase)](https://skillvault-tera-dark.vercel.app) *(注：云端部署为 Demo 演示版，所有操作仅在浏览器内存中生效；本地双击 run.bat 运行依然读取本地磁盘)*
...
1.  **局域网共享**:
    后端在启动时会扫描您的网卡并打印出局域网 IP。同局域网下的手机、平板或其他电脑可直接通过 `http://<局域网IP>:3000` (或前端打包后的共享端口) 进行文档的实时跨端查看；
2.  **端口冲突降级 (EACCES)**:
    如果系统局域网广播端口冲突或无权限绑定，后端 `server.js` 会自动降级绑定到本地回环地址 `127.0.0.1:23333`，并在控制台给出明确警告，确保单机模式稳定运行。
